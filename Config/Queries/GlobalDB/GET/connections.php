...
return [
    'all' => [
        'query' => "SELECT * FROM `{$this->globalDB}`.`{$Env::$connections}` WHERE __WHERE__ ORDER BY connection_id ASC",
        '__WHERE__' => [
    ...
    'single' => [
        'query' => "SELECT * FROM `{$this->globalDB}`.`{$Env::$connections}` WHERE __WHERE__",
